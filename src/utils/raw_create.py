...
if __name__ == "__main__":
  ...
  if with_bccwj:
    tags = ["train", "dev", "test"]
    listpath = {"train": Path("../../data/BCCWJ-DepParaPAS/file_train.txt"),
                "dev": Path("../../data/BCCWJ-DepParaPAS/file_dev.txt"),
                "test": Path("../../data/BCCWJ-DepParaPAS/file_test.txt")}
    filelists = {}
    for tag in tags:
    ...
    base_path = Path(
      "../../data/BCCWJ-DepParaPAS/BCCWJ-DepParaPAS-3.3.0_1.2.0_20160301/BCCWJ-DepParaPAS-3.3.0_1.2.0_20160301/ntc/raw")
    ...
    texts = {}
    with Path('../../data/BCCWJ-DepParaPAS/raw_listed.txt').open('w', encoding='utf-8') as f:
      for mode, ds in zip(["train", "dev", "test"], [train, dev, test]):
        for filename, sentences in ds.items():
          for _, sentence in sentences.items():
            text = []
            for _, words in sentence.items():
              for _, word in words.items():
                text.append(word['info'][0])
            f.write(','.join([mode, filename, ''.join(text)]))
            f.write('\n')
            texts[''.join(text)] = (filename, mode)
    with Path('../../data/BCCWJ-DepParaPAS/raw_dict.txt').open('w', encoding='utf-8') as f:
    ...
    base_path = Path("../../data/NTC_dataset")
