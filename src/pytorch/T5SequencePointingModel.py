...
class T5SequencePointingModel(Model):
    def __init__(self, word_pos_size=20,
                 ...
                 trainbert=False,
                 with_bccwj=False):
        super(T5SequencePointingModel, self).__init__()
        ...
        self.word_embeddings = T5SPModel(device=device, trainable=trainbert)
...
class T5SimpleSequencePointingModel(T5SequencePointingModel):
    def __init__(self, word_pos_size=20,
                 ku_pos_size=20,
                 mode_size=2,
                 target_size=4,
                 ...
                 trainbert=False,
                 with_bccwj=False):
        super(T5SimpleSequencePointingModel, self).__init__()
        torch.manual_seed(seed)
    ...
    def forward(self, arg, pred, word_pos, ku_pos, mode, tag, is_wsc=False):
        ...
        if is_wsc:
            ...
            embedding = self.word_embeddings.model(input_ids=input_ids, attention_mask=attention_mask, output_hidden_states=True, decoder_input_ids=input_ids)
            arg_embeds = embedding.encoder_last_hidden_state
...
if __name__ == "__main__":
    model = T5SequencePointingModel(trainbert=False)
