...
class BertSimpleSequencePointingModel(Model):
    def __init__(self, word_pos_size=20,
                 ku_pos_size=20,
                 mode_size=2,
                 target_size=4,
                 ...
                 bidirectional=True, return_seq=False,
                 batch_first=True, continue_seq=False):
        super(BertSimpleSequencePointingModel, self).__init__()
        torch.manual_seed(seed)
        ...
        self.word_embeddings = AutoModelForCausalLM.from_pretrained("cl-tohoku/bert-base-japanese-whole-word-masking").to(device)
        # self.word_embeddings = BertWithJumanModel(device=device)
        self.embedding_dim = self.word_embeddings.bert.config.hidden_size
        self.vocab_padding_idx = self.word_embeddings.bert.config.pad_token_id
    ...
    def forward(self, arg, pred, word_pos, ku_pos, mode, is_wsc=True):
        # output shape: Batch, Sentence_length, word_embed_size
        if is_wsc:
            input_ids = torch.LongTensor(arg).to(self.device)
            attention_mask = torch.LongTensor([[1] * len(arg[0])]).to(self.device)
            embedding = self.word_embeddings(input_ids=input_ids, attention_mask=attention_mask, output_hidden_states=True)
