...
class SimplePointerNetworksModel(PointerNetworksModel):
    def __init__(self, vocab_size, target_size,
                 device, with_linear=False,
                 embedding_dim=32,
                 vocab_padding_idx=-1,
                 vocab_null_idx=-1, vocab_unk_idx=-1,
                 num_layers=1, dropout_ratio=.1,
    ...
    def forward(self, arg, pred, word_pos, ku_pos, mode):
        ...
        arg_embeds = self.word_embeddings(input_ids)
        # arg_embeds = self.vocab_zero_padding(arg, arg_embeds)
        # pred_embeds = self.word_embeddings(pred)
        # pred_embeds = self.vocab_zero_padding(pred, pred_embeds)
        if 1 not in pred[0]:
            print(pred)
        pred_embeds = torch.stack(arg_embeds.shape[1] * [arg_embeds[0][pred[0].index(1)]], dim=1).unsqueeze(
            0).transpose(1, 2)

        # output shape: Batch, Sentence_length, 2 * pos_embed_size
        concatten_embeds = torch.cat((arg_embeds, pred_embeds), 2)
