...
from utils.WSCDataset import WSCBERTDataset
...
from BertSequencePointingModel import BertSimpleSequencePointingModel
...
def train(batch_size, learning_rate=1e-3, fc1_size=128, optim="adam",  dropout_ratio=0.4, null_weight=None, loss_weight=None, norm_type=None):
    ...
    model = BertSimpleSequencePointingModel(target_size=1,
    ...
    wsc_train_batcher = DataLoader(WSCBERTDataset(mode='train'), batch_size=1, shuffle=True)
    wsc_dev_batcher = DataLoader(WSCBERTDataset(mode='dev'), batch_size=1, shuffle=False)
    wsc_test_batcher = DataLoader(WSCBERTDataset(mode='test'), batch_size=1, shuffle=False)
    ...
    for e in range(arguments.epochs):
        ...
        with Path(f'./wsc_bert_{token_pooling_method}_result.txt').open('w') as f:
