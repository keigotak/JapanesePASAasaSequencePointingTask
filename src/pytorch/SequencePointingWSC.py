...
from utils.WSCDataset import WSCMecabDataset
...
from PointerNetworksModel import SimplePointerNetworksModel
...
def train(batch_size, learning_rate=1e-3, fc1_size=128, optim="adam",  dropout_ratio=0.4, null_weight=None, loss_weight=None, norm_type=None):
    ...
    wsc_train_batcher = DataLoader(WSCMecabDataset(mode='train'), batch_size=1, shuffle=True)
    wsc_dev_batcher = DataLoader(WSCMecabDataset(mode='dev'), batch_size=1, shuffle=False)
    wsc_test_batcher = DataLoader(WSCMecabDataset(mode='test'), batch_size=1, shuffle=False)

    model = SimplePointerNetworksModel(target_size=1,
                                       vocab_size=len(wsc_train_batcher.dataset.word_to_id),
                                       embedding_dim=300,
    ...
    for e in range(arguments.epochs):
        ...
        for data in wsc_train_batcher:
            ...
            ga_logits, ni_logits, wo_logits = model(arg=[[int(d[0]) for d in data[0]]],
                                                                ...
                                                                word_pos=None, ku_pos=None, mode=None)
        ...
        with Path(f'./wsc_raw_{token_pooling_method}result.txt').open('w') as f:
