...
from SequenceLabelingModel import SequenceLabelingModel
...
def train(batch_size, learning_rate=1e-3, optim="adam",  dropout_ratio=0.4, null_weight=None, loss_weight=None, norm_type=None):
    ...
    if arguments.embed == 'original':
        model = SequenceLabelingModel(target_size=4,
    ...
    if null_weight is not None:
        ...
        criterion = nn.CrossEntropyLoss(ignore_index=PADDING_ID, weight=class_weight)
        ...
        for t_batch in range(len(train_batcher)):
            ...
            with torch.no_grad():
                for d_batch in range(len(dev_batcher)):
                    ...
                    dev_loss = criterion(prediction.view(-1, t_size), d_labels.view(-1))
...
if __name__ == "__main__":
    if arguments.hyp:
        # train(batch_size=10,
        #       learning_rate=0.0001, optim="rmsprop", dropout_ratio=0.2,
        #       norm_type={'clip': 10, 'weight_decay': 0.0})
        train(batch_size=2,
